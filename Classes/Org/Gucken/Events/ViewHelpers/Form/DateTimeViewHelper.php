...
class DateTimeViewHelper extends AbstractFormFieldViewHelper
{
    ...
    public function render(
        ...
        $this->tagTime->setContent(
            $this->renderOptions($this->generateTimeOptions($timeFormat), $value->format($timeFormat))
        );
    ...
    protected function renderOptions($options, $selected)
    {
        ...
        foreach ($options as $value => $label) {
            $optionString .= '<option value="' . htmlspecialchars(
                    $value,
                    ENT_QUOTES,
                    'UTF-8'
                ) . '"' . ($value == $selected ? ' selected="selected"' : '') . '>' . htmlspecialchars(
                    $label,
                    ENT_QUOTES,
                    'UTF-8'
                ) . '</option>' . PHP_EOL;
